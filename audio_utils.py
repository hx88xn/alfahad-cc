...
The Al Fardan browser client sends/plays:
- 8kHz linear 16-bit PCM (the same wire format the OpenAI path decodes before
  re-encoding to mulaw). So for Gemini we resample 8k->16k inbound and the model's
  24k output is sent to the browser as-is (the client reads the per-message
  sampleRate) or downsampled 24k->8k via convert_gemini_to_browser.
"""
